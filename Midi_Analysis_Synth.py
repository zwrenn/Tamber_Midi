...
def create_db():
    ...
    c = conn.cursor()
    c.execute('''
    CREATE TABLE IF NOT EXISTS SynthFeatures (
...
def insert_into_db(features, key_signature, conn, table_name='SynthFeatures'):
    c = conn.cursor()
...
def main():
    ...
    for key_folder in os.listdir(root_folder):
        ...
        if os.path.isdir(key_path):
            for inst_folder in ['Synth']:  # Adding a loop to go through Synth
                inst_path = os.path.join(key_path, inst_folder)
                if os.path.exists(inst_path):
                    for midi_file in get_midi_files(inst_path):
                        features = analyze_midi(midi_file)
                        if features:
                            insert_into_db(features, key_folder, conn, 'SynthFeatures')  # Storing both in SynthFeatures
    conn.commit()
    conn.close()
