...
def analyze_midi(midi_file_path):
    ...
    tempo_match = re.search(r'(\d+)\s*bpm\b', features['file_name'], re.IGNORECASE)
    if tempo_match:
...
def create_db():
    ...
    c = conn.cursor()
    c.execute('''
    CREATE TABLE IF NOT EXISTS PadFeatures (
...
def insert_into_db(features, key_signature, conn):
    c = conn.cursor()
    ...
    c.execute("INSERT INTO PadFeatures (file_name, key_signature, total_notes, unique_notes, note_length_mean, note_length_std, velocity_mean, velocity_std, start_time_mean, start_time_std, end_time_mean, end_time_std, pitch_classes, chord_progression, tempo, time_signature, key_scale, modality, harmony_complexity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
def main():
    ...
    for key_folder in os.listdir(root_folder):
        ...
        if os.path.isdir(key_path):
            pad_folder = os.path.join(key_path, 'Pad')
            if os.path.exists(pad_folder):
                for midi_file in get_midi_files(pad_folder):
                    features = analyze_midi(midi_file)
                    if features:
                        insert_into_db(features, key_folder, conn)
    conn.commit()
    conn.close()
